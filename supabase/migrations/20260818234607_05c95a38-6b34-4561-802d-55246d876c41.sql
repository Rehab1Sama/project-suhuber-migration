...
create type public.app_role as enum ('platform_owner','tenant_admin','admin_deputy','academic_deputy','supervisor','teacher','student');
...
returns boolean language sql stable security definer set search_path = public as $$
  select exists (select 1 from public.user_roles r where r.user_id = _user_id and r.role = 'platform_owner');
$$;

create or replace function public.is_tenant_member(_user_id uuid, _tenant_id uuid)
returns boolean language sql stable security definer set search_path = public as $$
  select exists (select 1 from public.user_roles r where r.user_id = _user_id and r.tenant_id = _tenant_id)
...
returns boolean language sql stable security definer set search_path = public as $$
  select exists (
    select 1 from public.user_roles r
    where r.user_id = _user_id and r.tenant_id = _tenant_id and r.role = any(_roles)
  ) or public.is_platform_owner(_user_id);
$$;

create or replace function public.can_record_academic(_user_id uuid, _tenant_id uuid)
returns boolean language sql stable security definer set search_path = public as $$
  select public.has_tenant_role(_user_id, _tenant_id,
...
returns boolean language sql stable security definer set search_path = public as $$
  select coalesce(
    (select tf.enabled from public.tenant_features tf where tf.tenant_id = _tenant_id and tf.feature_key = _feature_key),
...
declare
  ...
  _usage integer;
begin
  select case _kind when 'students' then l.max_students when 'circles' then l.max_circles when 'teachers' then l.max_teachers else null end
  into _limit from public.tenant_plan_limits(_tenant_id) l;

  if _limit is null then return true; end if;
  if _limit = 0 then return true; end if;

  select case _kind when 'students' then u.students when 'circles' then u.circles when 'teachers' then u.teachers else 0 end
  into _usage from public.tenant_usage(_tenant_id) u;
...
begin
  for t in
    select unnest(array['profiles','tenants','plans','features','tenant_features','tracks','students',
...
create policy "tenants_public_read_active" on public.tenants for select to anon, authenticated
  using (status = 'active');
...
create policy "tenants_manager_update" on public.tenants for update to authenticated
  using (public.is_tenant_manager(auth.uid(), id)) with check (public.is_tenant_manager(auth.uid(), id));

grant select, insert, update, delete on public.user_roles to authenticated;
grant all on public.user_roles to service_role;
alter table public.user_roles enable row level security;
create policy "user_roles_select_own_or_tenant" on public.user_roles for select to authenticated
...
create policy "plans_owner_read_all" on public.plans for select to authenticated
...
create policy "features_public_read" on public.features for select to anon, authenticated using (true);
create policy "features_owner_manage" on public.features for all to authenticated
  using (public.is_platform_owner(auth.uid())) with check (public.is_platform_owner(auth.uid()));
...
create policy "tenant_features_owner_manage" on public.tenant_features for all to authenticated
  using (public.is_platform_owner(auth.uid())) with check (public.is_platform_owner(auth.uid()));

grant select, insert, update, delete on public.tracks to authenticated;
grant all on public.tracks to service_role;
alter table public.tracks enable row level security;
create policy "tracks_member_read" on public.tracks for select to authenticated
  using (public.is_tenant_member(auth.uid(), tenant_id));
create policy "tracks_manager_write" on public.tracks for all to authenticated
...
create policy "students_member_read" on public.students for select to authenticated
  using (public.is_tenant_member(auth.uid(), tenant_id));
...
create policy "circles_member_read" on public.circles for select to authenticated
  using (public.is_tenant_member(auth.uid(), tenant_id));
create policy "circles_manager_write" on public.circles for all to authenticated
  using (public.is_tenant_manager(auth.uid(), tenant_id))
  with check (public.is_tenant_manager(auth.uid(), tenant_id));
...
create policy "circle_students_member_read" on public.circle_students for select to authenticated
  using (exists (select 1 from public.circles c where c.id = circle_students.circle_id
    and public.is_tenant_member(auth.uid(), c.tenant_id)));
create policy "circle_students_manager_write" on public.circle_students for all to authenticated
  using (exists (select 1 from public.circles c where c.id = circle_students.circle_id
    and public.is_tenant_manager(auth.uid(), c.tenant_id)))
  with check (exists (select 1 from public.circles c where c.id = circle_students.circle_id
    and public.is_tenant_manager(auth.uid(), c.tenant_id)));

grant select, insert, update, delete on public.attendance to authenticated;
grant all on public.attendance to service_role;
alter table public.attendance enable row level security;
create policy "attendance_member_read" on public.attendance for select to authenticated
  using (public.is_tenant_member(auth.uid(), tenant_id));
create policy "attendance_recorder_write" on public.attendance for all to authenticated
  using (public.can_record_academic(auth.uid(), tenant_id))
  with check (public.can_record_academic(auth.uid(), tenant_id));
...
create policy "progress_records_member_read" on public.progress_records for select to authenticated
  using (public.is_tenant_member(auth.uid(), tenant_id));
create policy "progress_records_recorder_write" on public.progress_records for all to authenticated
  using (public.can_record_academic(auth.uid(), tenant_id))
  with check (public.can_record_academic(auth.uid(), tenant_id));
...
create policy "quotas_member_read" on public.quotas for select to authenticated
  using (public.is_tenant_member(auth.uid(), tenant_id));
create policy "quotas_manager_write" on public.quotas for all to authenticated
  using (public.is_tenant_manager(auth.uid(), tenant_id))
  with check (public.is_tenant_manager(auth.uid(), tenant_id));
...
create policy "invitations_manager_read" on public.invitations for select to authenticated
  using (public.is_platform_owner(auth.uid())
    or (tenant_id is not null and public.is_tenant_manager(auth.uid(), tenant_id))
    or lower(email) = lower(coalesce((auth.jwt() ->> 'email'), '')));
create policy "invitations_manager_manage" on public.invitations for all to authenticated
  using (public.is_platform_owner(auth.uid())
    or (tenant_id is not null and public.is_tenant_manager(auth.uid(), tenant_id)))
...
create policy "contact_messages_owner_manage" on public.contact_messages for all to authenticated
  using (public.is_platform_owner(auth.uid())) with check (public.is_platform_owner(auth.uid()));
...
create policy "plan_requests_anon_insert" on public.plan_requests for insert to anon with check (true);
create policy "plan_requests_auth_insert" on public.plan_requests for insert to authenticated with check (true);
create policy "plan_requests_owner_manage" on public.plan_requests for all to authenticated
  using (public.is_platform_owner(auth.uid())) with check (public.is_platform_owner(auth.uid()));
create policy "plan_requests_tenant_read" on public.plan_requests for select to authenticated
...
create policy "subscriptions_tenant_read" on public.subscriptions for select to authenticated
  using (public.is_tenant_member(auth.uid(), tenant_id));
create policy "subscriptions_owner_manage" on public.subscriptions for all to authenticated
  using (public.is_platform_owner(auth.uid())) with check (public.is_platform_owner(auth.uid()));

grant select, insert, update, delete on public.payment_intents to authenticated;
grant all on public.payment_intents to service_role;
alter table public.payment_intents enable row level security;
create policy "payment_intents_tenant_read" on public.payment_intents for select to authenticated
  using ((tenant_id is not null and public.is_tenant_manager(auth.uid(), tenant_id))
    or created_by = auth.uid() or public.is_platform_owner(auth.uid()));
create policy "payment_intents_authenticated_insert" on public.payment_intents for insert to authenticated
  with check (created_by = auth.uid() or public.is_platform_owner(auth.uid()));
create policy "payment_intents_owner_manage" on public.payment_intents for all to authenticated
  using (public.is_platform_owner(auth.uid())) with check (public.is_platform_owner(auth.uid()));
...
create policy "invoices_owner_manage" on public.invoices for all to authenticated
  using (public.is_platform_owner(auth.uid())) with check (public.is_platform_owner(auth.uid()));

grant select, insert, update, delete on public.payment_webhook_events to authenticated;
grant all on public.payment_webhook_events to service_role;
alter table public.payment_webhook_events enable row level security;
create policy "payment_webhook_events_owner_manage" on public.payment_webhook_events for all to authenticated
  using (public.is_platform_owner(auth.uid())) with check (public.is_platform_owner(auth.uid()));
